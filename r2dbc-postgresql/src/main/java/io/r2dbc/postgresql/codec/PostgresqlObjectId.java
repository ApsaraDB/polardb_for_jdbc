...
public enum PostgresqlObjectId implements Type, PostgresTypeIdentifier {
    ...
    /**
     * PolarDB date object id.
     * PolarDB uses OID 9002 for its custom date type instead of the standard PostgreSQL DATE (1082).
     * The format is identical to TIMESTAMP and can be parsed the same way.
     */
    POLARDB_DATE(9002, LocalDate.class),

    /**
     * PolarDB date array object id.
     */
    POLARDB_DATE_ARRAY(9003, LocalDate[].class);

    public static final int OID_CACHE_SIZE = 9010; // POLARDB_DATE_ARRAY is currently the highest one
