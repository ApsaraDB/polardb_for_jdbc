...
import static io.r2dbc.postgresql.codec.PostgresqlObjectId.POLARDB_DATE_ARRAY;
...
final class LocalDateCodec extends AbstractTemporalCodec<LocalDate> {
    ...
    LocalDateCodec(ByteBufAllocator byteBufAllocator, boolean usePolarDbDate) {
        super(LocalDate.class, byteBufAllocator,
            usePolarDbDate ? POLARDB_DATE : DATE,
            usePolarDbDate ? POLARDB_DATE_ARRAY : DATE_ARRAY,
            usePolarDbDate
                ? (date -> PostgresqlDateTimeFormatter.toString(date.atStartOfDay()))
                : PostgresqlDateTimeFormatter::toString);
    }
