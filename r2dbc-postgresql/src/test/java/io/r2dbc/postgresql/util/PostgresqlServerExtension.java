...
public final class PostgresqlServerExtension implements BeforeAllCallback, AfterAllCallback {
    ...
    private void initializeConnectors() {
        ...
        dataSource.setJdbcUrl(String.format("jdbc:polardb://%s:%d/%s?prepareThreshold=1", getHost(), getPort(), getDatabase()));
     ...
     * Reads connection details from the {@code .testrc} properties file.
     */
    static class External implements DatabaseContainer {
        ...
        External(Properties properties) {
            ...
            this.port = Integer.parseInt(properties.getProperty("port", "5432"));
            this.database = properties.getProperty("database", "postgres");
            this.username = properties.getProperty("username", "postgres");
            this.password = properties.getProperty("password", "postgres");
        }
