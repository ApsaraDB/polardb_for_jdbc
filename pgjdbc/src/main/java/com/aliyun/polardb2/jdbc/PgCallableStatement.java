...
class PgCallableStatement extends PgPreparedStatement implements CallableStatement {
  ...
  /* POLAR: CALL statements return OUT parameters as a synthetic result row. */
  ...
  PgCallableStatement(PgConnection connection, String sql, int rsType, int rsConcurrency,
    ...
    String nativeSql = preparedQuery.query.getNativeSql().trim();
    this.isProcedureCall = nativeSql.length() > 4
        && nativeSql.regionMatches(true, 0, "call", 0, 4)
        && Character.isWhitespace(nativeSql.charAt(4));
  ...
  public boolean executeWithFlags(int flags) throws SQLException {
    try (ResourceLock ignore = lock.obtain()) {
      ...
      /* POLAR: Fall through for CALL statements even when no out parameter was
       * registered (returnTypeSet == false). The server returns actual OUT/INOUT
       * parameters as a one-row result set, including parameters that the caller
       * bound only as IN. Oracle executes such calls but never exposes that row as
       * a regular result set. Consume it into callResult and report "no results" so
       * frameworks like MyBatis behave the same as with the Oracle driver instead
       * of requiring an unrelated result map. Keep exposing results for JDBC calls
       * rewritten to SELECT, since those are regular function result sets. */
      if (!isFunction || functionReturnType == null
          || (!returnTypeSet && !isDoBlock && !isProcedureCall)) {
        return hasResultSet;
      }
