...
import com.aliyun.polardb2.util.PGTime;
import com.aliyun.polardb2.util.PGTimestamp;
...
import com.aliyun.polardb2.util.ReaderInputStream;
...
class PgPreparedStatement extends PgStatement implements PreparedStatement {
  ...
  public void setNull(int parameterIndex, int sqlType) throws SQLException {
    ...
    // consider ? = func(?, ?)
    if (parameterIndex < 1 || parameterIndex > preparedParameters.getParameterCount() + 1) {
      throw new PSQLException(
  ...
  public void setArray(int i, java.sql.@Nullable Array x) throws SQLException {
    ...
    if (x instanceof PgArray) {
      // Use the OID already resolved during createArrayOf — avoids
      // round-trip re-resolution that breaks TABLE OF types.
    ...
    if (oid == Oid.UNSPECIFIED) {
      throw new PSQLException(GT.tr("Unknown type {0}.", x.getBaseTypeName()),
          PSQLState.INVALID_PARAMETER_TYPE);
    }
